...
- code：派发 coder；传递 progressive context manifest；Core 根据 Git diff 生成证据映射。
- test：调用一次 `lifecycle(verify_delivery)`；成功后展示候选，用户确认才 finalize。
...
- `sdlc_lifecycle(init|focused_check|verify_delivery)`：隐藏内部生命周期；
- `sdlc_finalize`：用户确认后固化版本。

判断交给 AI：是否需要额外阻塞问题、模块实现、读取哪些资源、选择哪些功能测试键。
