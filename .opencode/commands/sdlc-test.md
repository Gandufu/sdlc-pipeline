...
仅派发一次 `sdlc-executor`。executor 通过 `sdlc_lifecycle(action=run_tests)` 执行计划并返回
逐 T-id handoff；校验后主会话调用 `sdlc_lifecycle(action=test)` 复用 runner evidence，
生成固定格式结果和精简摘要，不重复执行测试命令。
